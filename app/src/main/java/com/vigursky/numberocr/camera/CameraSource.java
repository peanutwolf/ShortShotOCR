 ...
 */
package com.vigursky.numberocr.camera;
...
import com.vigursky.numberocr.FocusBoxView;
import com.vigursky.numberocr.Tools;
...
public class CameraSource {
    ...
    private FocusBoxView.FocusBoxTouchListener mFocusBoxListener = null;
    ...
    private FocusBoxView mFocusBox = null;
    ...
    public static class Builder {
        ...
        public Builder setFocusBox(FocusBoxView focusBox){
            mCameraSource.mFocusBox = focusBox;
            return this;
        }
    ...
    public CameraSource updatePreviewSurface(SurfaceHolder surfaceHolder)  throws IOException{
        synchronized (mCameraLock) {
            if (mCamera == null) {
                return null;
            }
    ...
    private CameraSource() {
        mFocusBoxListener = (Rect boxRect, MotionEvent event) -> {
            if(mCamera == null)
                return;
            Camera.Parameters parameters = mCamera.getParameters();
            if (parameters.getMaxNumMeteringAreas() <= 0)
                return;
            cancelAutoFocus();
            Rect focusArea = calculateFocusArea(boxRect);
            Tools.transpone(focusArea, getRotation(), 2000, 2000);
            Tools.resize(focusArea, -1000);


//                    Tools.bound(focusArea, 1000, new BiPredicate<Integer, Integer>() {
//                        @Override
...
//                    });
            ...
            try {
                Log.d(TAG, "Set focusArea for rect = [" + focusArea + "]");
//                mCamera.setParameters(parameters);
            }catch (Exception e){
                Log.d(TAG, "Failed to set focusArea for rect = [" + focusArea + "]");
                return;
            }

            autoFocus((result) ->{
                Log.d(TAG, "AutoFocus for x=[" + event.getX() +"], y=["+ event.getY() +"]" +
                        "focusArea="+focusArea.toShortString() +
                        ", result=["+result+"]");
    ...
    private Rect calculateFocusArea(Rect boxRect) {
        double kx = (double)mFocusBox.getScreenResolution().x / 2000D;
        double ky = (double)mFocusBox.getScreenResolution().y / 2000D;
        int left = (int)((double)boxRect.left * kx);
        int top = (int)((double)boxRect.top * ky);
        int right = (int)((double)boxRect.right * kx);
        int bottom = (int)((double)boxRect.bottom * ky);
    ...
    public class PendingFrame {
        ...
        public ByteBuffer mPendingFrameData = null;
    ...
    private PendingFrame mPendingFrame = new PendingFrame();

    private void setNextFrame(byte[] data, Camera camera) {
        synchronized (this) {
            if (mPendingFrame.mPendingFrameData != null) {
                camera.addCallbackBuffer(mPendingFrame.mPendingFrameData.array());
                mPendingFrame.mPendingFrameData = null;
            }
            ...
            mPendingFrame.mPendingTimeMillis = SystemClock.elapsedRealtime() - mPendingFrame.mStartTimeMillis;
            mPendingFrame.mPendingFrameId++;
            mPendingFrame.mPendingFrameData = mBytesToByteBuffer.get(data);
    ...
    public PendingFrame getNextFrame(){
        synchronized (this){
    ...
    public void pauseCameraFraming(boolean pause) {
        ...
        synchronized (this){
            ...
            if(pause){
                ...
                Iterator<byte []> prevIter = mBytesToByteBuffer.keySet().iterator();
                mCamera.setPreviewCallbackWithBuffer(new CameraPreviewCallback());
                for(int i = 0; i < 4;  i++){
                    if(prevIter.hasNext()){
                        mCamera.addCallbackBuffer(prevIter.next());
                    }else{
